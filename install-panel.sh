...
user_password=`sudo pwgen -n 20 -1 | tr -d "&" | tee /root/PTERODACTYL_PASSWORD`
...
GITHUB_BASE_URL="https://raw.githubusercontent.com/ikoula/pterodactyl-installer/$GITHUB_SOURCE"
...
main() {
  ...
  FQDN=`cat /root/FQDN`
  SERVER_PUB_IP=`/usr/sbin/ip -4 addr | sed -ne 's|^.* inet \([^/]*\)/.* scope global.*$|\1|p' | head -1`
